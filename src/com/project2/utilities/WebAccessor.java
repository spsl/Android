package com.project2.utilities;
...
public class WebAccessor {
	 ...
	 * update order view
	 */
	public boolean getAllWebOrders(String orderUser, Identity identity) {	
		...
		try {
			...
			// Get all old orders to determine if order updated.
			HashMap<String, String> oldOrders = new LinkedHashMap<String, String>();
			Cursor cursor = getOrderCursor(orderUser, identity);
            cursor.moveToFirst();
            while (!cursor.isAfterLast()) {
            	oldOrders.put(cursor.getString(0), cursor.getString(1));
                cursor.moveToNext();
            }
			...
			while ((line = reader.readLine()) != null) {
				...
				values.put(MySQLiteHelper.COLUMN_ORDERITEM, orderStrings[3]);
				...
				if (statusUpdated == false) {
					if (!oldOrders.containsKey(orderStrings[0])) {
						statusUpdated = true;
					} else {				
						for (String orderID : oldOrders.keySet()) {
							if (orderID.equals(orderStrings[0])) {
								String oldStatus = oldOrders.get(orderID);
								if (!oldStatus.equals(orderStrings[1])) {
									statusUpdated = true;
									break;
								}
							}
	 ...
	 */
	public int addOrder(String orderUser, String orderItem) {
		// Create a new HttpClient and Post Header
		HttpClient httpclient = new DefaultHttpClient();
		...
		try {
			...
			nameValuePairs.add(new BasicNameValuePair("orderItem", orderItem));
			...
			values.put(MySQLiteHelper.COLUMN_ORDERITEM, orderItem);
